...
fn test_{{ctx.name}}() {
    ...
    let mut memory = Memory::<SIZE>::new_zeroed();
    ...
    let dut = unsafe { {{ctx.name}}::{{ctx.type_name}}::from_ptr(memory.as_mut().as_mut_ptr() as _) };

    {% for field in ctx.fields %}
        {% for pattern in field.test_patterns %}
    // Test {{field.name}} with pattern: {{pattern.description}}
    {% if pattern.raw_write %}
    memory.at_mut(0x{{"%X" % field.address}}).write_field::<{{field.primitive}}>({{field.bit_offset}}..{{field.bit_offset + field.width}}, {{pattern.raw_value}});
    {% else %}
    ...
    assert_eq!(dut.{{field.reg_method}}.read().{{field.name}}(), {{pattern.value}});
        {% endif %}
    {% endif %}
    let mem_val = memory.at(0x{{"%X" % field.address}}).read_field::<{{field.primitive}}>({{field.bit_offset}}..{{field.bit_offset + field.width}});
